...
const AccData = (props) => {
  ...
  const deletephotoHandler = () => {
    ...
    const updatedData = {
      ...
      password: localData[props.activeUser]?.password || "",
      phone: localData[props.activeUser]?.phone || "",
      // No profilePic property to remove
    };
    const updatedLocalStorage = {
      ...localData,
      [props.activeUser]: updatedData,
    };

    localStorage.setItem("accountsPage", JSON.stringify(updatedLocalStorage));

  };

  const uploadPhotoHandler = () => {
    // Assuming you have an API call to fetch a new photo
    // Replace the following with your actual API call
    fetch("https://your-api-endpoint.com/photo")
      .then((response) => response.json())
      .then((data) => {
        setAccpic(data.photoUrl); // Set the retrieved photo URL to state
      })
      .catch((error) => {
  ...
  useEffect(() => {
    if (props.activeUser) {
      const selectedDataToShow = localData[props.activeUser];
      setAccname(selectedDataToShow?.name || "");
      // ... other state updates ...
    }
  }, [props.activeUser]);
        ...
        <div className={styles.accountdatacontainer}>
          <div className={styles.imgcontainer}>
            ...
            <div className={styles.imgoverlay}>
              ...
              <MdDelete onClick={deletephotoHandler}  size={'20px'}/>

            </div>
          </div>
          <button className="btn" onClick={uploadPhotoHandler}>Upload New Photo</button>
        </div>
        <div className={styles.accountdatacontainer}>
